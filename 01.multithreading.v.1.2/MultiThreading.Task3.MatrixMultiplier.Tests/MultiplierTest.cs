    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            for (long i = 1; i < long.MaxValue; i++)
            {
                ...
                long matricesMultiplierExecutionTime = watch.ElapsedMilliseconds;
                watch.Stop();

                watch.Start();
                matricesMultiplierParallel.Multiply(m1, m2);
                long matricesMultiplierParallelExecutionTime = watch.ElapsedMilliseconds;
                watch.Stop();
                Console.WriteLine(matrixExecutionPrevail);
                if (matricesMultiplierParallelExecutionTime < matricesMultiplierExecutionTime)
                {
                    matrixExecutionPrevail = matricesMultiplierParallelExecutionTime;
                    break;
                }
            }
